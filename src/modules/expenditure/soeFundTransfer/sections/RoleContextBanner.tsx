/* ═══════════════════════════════════════════════════════════════════════════
   RoleContextBanner — SOE & Fund Transfer Management
...
import {
  ...
  soeToneClasses,
} from "../state/useSoeRoleCapabilities";

export function RoleContextBanner() {
  const caps = useSoeRoleCapabilities();
  const tone = soeToneClasses(caps.personaTone);
    ...
    >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        <CapabilityList
          ...
          emptyMessage="Nothing is locked — full SOE & Fund Transfer access."
        />
      </div>

      <div className="mt-3 flex flex-wrap items-center gap-1.5">
        <FlagPill label="Create transfer" enabled={caps.canCreate} />
        <FlagPill label="Edit header (Step 1)" enabled={caps.canEditStep(1)} />
        <FlagPill label="Attach SoE (Step 2)" enabled={caps.canAttachSoe} />
        <FlagPill label="Validate / approve (Step 3)" enabled={caps.canValidate || caps.canApprove} />
        <FlagPill label="Release funds (Step 4)" enabled={caps.canRelease} />
        <FlagPill label="Reconcile (Step 5)" enabled={caps.canReconcile} />
