/* ═══════════════════════════════════════════════════════════════════════════
   RoleContextBanner
   ─────────────────
   Persistent header banner that shows the current "Acting as" persona, what
   they can do inside the Debt Payment module, and what is locked. Re-renders
   automatically when the user picks a different role from the top-bar
   switcher (AuthContext.activeRoleId).
   ═══════════════════════════════════════════════════════════════════════════ */
import {
  useDebtRoleCapabilities,
  debtToneClasses,
} from "../state/useDebtRoleCapabilities";

export function RoleContextBanner() {
  const caps = useDebtRoleCapabilities();
  const tone = debtToneClasses(caps.personaTone);
    ...
    >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        <CapabilityList
          ...
          emptyMessage="Nothing is locked — full Debt Payment access."
        />
      </div>

      {/* Quick capability flags row — explicit boolean state for each verb */}
      <div className="mt-3 flex flex-wrap items-center gap-1.5">
        <FlagPill label="Create record" enabled={caps.canCreate} />
        <FlagPill label="Edit header (Step 1)" enabled={caps.canEditStep(1)} />
        <FlagPill label="Payment orders (Step 2)" enabled={caps.canEditStep(2)} />
        <FlagPill label="Validate (Step 3)" enabled={caps.canValidate} />
        <FlagPill label="Authorise / Meridian (Step 4)" enabled={caps.canApprove} />
        <FlagPill label="Release payments (Step 5)" enabled={caps.canRelease} />
