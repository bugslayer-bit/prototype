/* ═══════════════════════════════════════════════════════════════════════════
   RoleContextBanner — Financial Institution Management
...
import {
  ...
  fiToneClasses,
} from "../state/useFiRoleCapabilities";

export function RoleContextBanner() {
  const caps = useFiRoleCapabilities();
  const tone = fiToneClasses(caps.personaTone);
    ...
    >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        <CapabilityList
          ...
          emptyMessage="Nothing is locked — full FI Management access."
        />
      </div>

      <div className="mt-3 flex flex-wrap items-center gap-1.5">
        <FlagPill label="Register FI" enabled={caps.canCreate} />
        <FlagPill label="Edit header (Step 1)" enabled={caps.canEditStep(1)} />
        <FlagPill label="Attach documents" enabled={caps.canAttachDocs} />
        <FlagPill label="Validate / approve (Step 2)" enabled={caps.canValidate || caps.canApprove} />
        <FlagPill label="Manage services (Step 3)" enabled={caps.canManageServices} />
        <FlagPill label="Monitoring (Step 4)" enabled={caps.canMonitor} />
        <FlagPill label="Lifecycle (Step 5)" enabled={caps.canLifecycle} />
