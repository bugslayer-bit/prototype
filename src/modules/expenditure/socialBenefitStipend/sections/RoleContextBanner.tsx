/* ═══════════════════════════════════════════════════════════════════════════
   RoleContextBanner — Social Benefits & Stipend Management
...
import {
  ...
  sbToneClasses,
} from "../state/useSbRoleCapabilities";

export function RoleContextBanner() {
  const caps = useSbRoleCapabilities();
  const tone = sbToneClasses(caps.personaTone);
    ...
    >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        <CapabilityList
          ...
          emptyMessage="Nothing is locked — full Social Benefit access."
        />
      </div>

      <div className="mt-3 flex flex-wrap items-center gap-1.5">
        <FlagPill label="Create program (Step 1)" enabled={caps.canCreate} />
        <FlagPill label="Onboard beneficiary (Step 2)" enabled={caps.canOnboardBeneficiary} />
        <FlagPill label="Manage deductions (Step 3)" enabled={caps.canManageDeductions} />
        <FlagPill label="Validate (Step 4)" enabled={caps.canValidate} />
        <FlagPill label="Approve (Step 4)" enabled={caps.canApprove} />
        <FlagPill label="Process payment (Step 5)" enabled={caps.canProcessPayment} />
        <FlagPill label="Release payment" enabled={caps.canReleasePayment} />
