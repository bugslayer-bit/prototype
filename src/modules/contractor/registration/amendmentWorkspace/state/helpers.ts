/* Pure helper functions for ContractorAmendmentWorkspace */
...
import type { FieldEditability } from "../types";
...
export function getAmendmentEditability(
  fieldKey: keyof FormState,
  kind: ContractorKind,
  currentValue: string,
): FieldEditability {
  const individualLockedFields: (keyof FormState)[] = [
    "contractorId",
    "contractorType",
    "dataSource",
    "registrationDate",
  ...
  const businessLockedFields: (keyof FormState)[] = [
    "contractorId",
    "contractorType",
    "dataSource",
    "registrationDate",
  ...
  const lockedFields = kind === "individual" ? individualLockedFields : businessLockedFields;
